...
const KEYWORDS = ['bitcoin', 'ethereum', 'cardano', 'solana', 'dogecoin']; // Add more keywords as you like
...
async function scrapeURLs(urls, keywords, startDate, endDate) {
  ...
  for (let keyword of keywords) {
    // Create an empty object to store the results for each keyword
    results[keyword] = {};
    ...
    while (currentDate.isSameOrBefore(endDate)) {
      ...
      for (let url of urls) {
        ...
        if (url === TWITTER_URL) { 
          ...
          // Store the result count for each URL in the results object
          results[keyword][formattedDate][url] = resultCount;
        }
