...
module.exports = class gateio extends Exchange {
        ...
        if (swap || future || option) {
            ...
            for (let c = 0; c < settlementCurrencies.length; c++) {
                ...
                for (let i = 0; i < response.length; i++) {
                    ...
                    const takerpct = this.safeString (market, 'taker_fee_rate');
                    const makerpct = this.safeString (market, 'maker_fee_rate', takerpct);
                    result.push ({
                        'id': id,
                        'symbol': symbol,
                        'base': base,
                        ...
                        'linear': (quote === settle),
                        'inverse': (base === settle),
                        'taker': this.parseNumber (Precise.stringDiv (takerpct, '100')), // Fee is in %, so divide by 100
                        'maker': this.parseNumber (Precise.stringDiv (makerpct, '100')),
            ...
            for (let i = 0; i < response.length; i++) {
                ...
                const base = this.safeCurrencyCode (baseId);
                const quote = this.safeCurrencyCode (quoteId);
                const takerpct = this.safeString (market, 'fee');
                const makerpct = this.safeString (market, 'maker_fee_rate', takerpct);
                ...
                result.push ({
                    ...
                    'taker': this.parseNumber (Precise.stringDiv (takerpct, '100')),
                    'maker': this.parseNumber (Precise.stringDiv (makerpct, '100')),
    ...
    parseTicker (ticker, market = undefined) {
        ...
        const pctage = this.safeString (ticker, 'change_pctage');
    ...
    parsePosition (position, market = undefined) {
        ...
        const pctage = Precise.stringMul (Precise.stringDiv (unrealisedPnl, initialMarginString), '100');
        return {
            'info': position,
            ...
            'initialMarginpctage': this.parseNumber (Precise.stringDiv (initialMarginString, notional)),
            'maintenanceMargin': this.parseNumber (Precise.stringMul (maintenanceRate, notional)),
            'maintenanceMarginpctage': this.parseNumber (maintenanceRate),
            ...
            'side': side,
            'pctage': this.parseNumber (pctage),
        };
    }
