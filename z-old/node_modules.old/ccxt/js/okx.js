...
module.exports = class okx extends Exchange {
    ...
    parsePosition (position, market = undefined) {
        ...
        let initialMarginpctage = undefined;
        ...
        if (marginType === 'cross') {
            ...
            initialMarginpctage = Precise.stringDiv ('1', leverageString);
            collateralString = this.safeString (position, 'margin');
        }
        const maintenanceMarginString = this.safeString (position, 'mmr');
        const maintenanceMargin = this.parseNumber (maintenanceMarginString);
        let maintenanceMarginpctage = Precise.stringDiv (maintenanceMarginString, notionalString);
        if (initialMarginpctage === undefined) {
            initialMarginpctage = this.parseNumber (Precise.stringDiv (initialMarginString, notionalString, 4));
        } else if (initialMarginString === undefined) {
            initialMarginString = Precise.stringMul (initialMarginpctage, notionalString);
        }
        const rounder = '0.00005'; // round to closest 0.01%
        maintenanceMarginpctage = this.parseNumber (Precise.stringDiv (Precise.stringAdd (maintenanceMarginpctage, rounder), '1', 4));
        const liquidationPrice = this.safeNumber (position, 'liqPx');
        const pctageString = this.safeString (position, 'uplRatio');
        const pctage = this.parseNumber (Precise.stringMul (pctageString, '100'));
